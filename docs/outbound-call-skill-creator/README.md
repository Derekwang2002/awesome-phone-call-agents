...
- reading approved Notion database or data source records for callbacks or follow-up tasks
- reading approved Airtable table or view records for callbacks or follow-up tasks
...
## Technical Binding Model

This section describes the generated-skill contract for maintainers and checkers. Do not mirror it as the first user prompt.
...
| `parameterized-bound` | Source family, access method, required schema, source-level outreach basis or consent rule, dedupe rule, goal contract, result-output policy, and result field schema. | Approved parameters such as form ID, Notion database or data source ID, Airtable base/table/view locator, CSV path, campaign ID, date window, source writeback target, source-adjacent artifact target, or output path. | Recommended default; balances reuse and automation. |
...
The creator should keep the setup flow guided and recommendation-first:

1. Ask for either the business workflow or the data source.
2. If only workflow is known, ask for the source family with examples such as `google-form`, `tiktok-ads`, `notion`, `airtable`, `local-csv`, or `other`.
...
For authenticated or connector-backed sources such as Google Forms, TikTok Ads, Notion, Airtable, and future OAuth, API, MCP, or managed connector sources, the creator collects only minimum connection details before access is verified. It must not present a blank manual mapping form for phone, recipient, consent, dedupe, goal inputs, or result-output fields before authentication and sample fetch have been attempted. After a redacted representative sample is available, it proposes the field mapping and asks the user to fill only fields that cannot be inferred.

If the user only names an authenticated source family such as `google-form`, `tiktok-ads`, `notion`, or `airtable`, first recommend a likely workflow and provisional call goal, then enter source access onboarding. The creator should proactively inspect host-local adapters, connectors, MCP tools, MCP resources, or helper scripts before asking the user to choose an access route. When a safe source authorization or auth-readiness action is available, start it before asking the user for another confirmation; do not ask the user to say `start auth`, choose a discovered route, or refresh a session before attempting the available non-mutating auth path. The creator must use the actual MCP-capable host or agent runtime selected by the user; it must not assume Codex. Host-specific commands are adapter examples, not universal instructions.

For Google Forms, when `google-form-callback` helpers are available, check `google-auth.mjs status`, run or request `preflight-auth.mjs --repair-google` if auth is missing, and use `google-local-api-client.mjs --action list-forms` before asking for a Form ID. For TikTok Ads, inspect exposed MCP tools and resources first. When the selected host is Codex and no TikTok Ads MCP server is configured, run `codex mcp add tiktok-ads --url https://business-api.tiktok.com/open_mcp/tt-ads-mcp-layer-tmp`, then `codex mcp get tiktok-ads` and `codex mcp list` before asking the user for a route choice. For Claude, Antigravity, Cursor, or another MCP-capable host, use that host's documented MCP server or connector setup and OAuth flow. Treat Codex `Auth: Unsupported` as missing Codex-managed OAuth only. If the route is configured but tools are not exposed, run `codex mcp login tiktok-ads` for Codex or the selected host's equivalent source MCP login before asking for a different route or session refresh. If TikTok Ads tools or resources are exposed, run a source-native read-only auth or inventory probe such as `auth_advertiser_get` before declaring a blocker; only ask for a supported token, managed connector, host-specific login path, or another approved route when no tools are exposed after the available auth path has been attempted or the source-native probe cannot authenticate.

For Notion, prefer hosted Notion MCP at `https://mcp.notion.com/mcp` because it uses OAuth and avoids user-managed integration tokens. Use the selected host's MCP server or connector setup and OAuth flow; when the selected host is Codex, configure it with `codex mcp add notion --url https://mcp.notion.com/mcp`, then run `codex mcp login notion` and refresh the session before sampling. Notion database locators must be resolved to canonical data source IDs before sampling, and Notion source writeback must stay blocked until hosted Notion MCP or another authenticated Notion route passes canary writeback/readback. For Airtable, prefer hosted Airtable MCP at `https://mcp.airtable.com/mcp` because it uses OAuth and avoids user-managed personal access tokens. Use the selected host's MCP server, connector, or plugin setup and OAuth flow; when the selected host is Codex, use `codex plugin add airtable@openai-curated` if plugin installation is allowed, or configure the route directly with `codex mcp add airtable --url https://mcp.airtable.com/mcp`, then run `codex mcp login airtable` and refresh the session before sampling. Airtable base, table, and view locators should resolve to stable IDs when schema access is available, and Airtable source writeback must stay blocked until hosted Airtable MCP or another authenticated Airtable route passes canary writeback/readback for non-destructive record updates. Ask the user only for the minimum locator or user-completed authorization step that remains necessary, and wait until the sample is available before asking for detailed field mapping, final goal-field selection, or result-output mapping.
...
- `skills/outbound-call-skill-creator/references/interaction-flow.md`: prompt order, known-value reuse, and copyable user choices
